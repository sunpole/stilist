...
class TypographyGame {
    constructor() {
        this.config = {
            ...
            orderGeneration: {
                minInterval: 2000,    // 2 секунды - минимальный интервал
                maxInterval: 5000,    // 5 секунд - максимальный интервал  
                maxBatchSize: 36,     // Максимум 36 заказов за один интервал
                maxAvailableOrders: 50 // Максимум одновременно доступных заказов
            },
    ...
    startGameLoop() {
        this.gameInterval = setInterval(() => {
            ...
            this.generateOrdersBatch(); // Новая функция пакетной генерации
        }, 1000);
    ...
    // НОВАЯ ФУНКЦИЯ: Пакетная генерация заказов
    generateOrdersBatch() {
        ...
        // Проверяем, пора ли генерировать новые заказы
        ...
        // Рассчитываем сколько заказов сгенерировать в этой партии
        ...
        if (progress < timeProgress * 0.7) { // Отстаем от графика
            batchSize = Math.min(batchSize * 2, this.config.orderGeneration.maxBatchSize);
        }
    ...
    // НОВАЯ ФУНКЦИЯ: Генерация одного заказа
    generateSingleOrder() {
        if (this.state.dailyOrders.generatedCount >= this.state.dailyOrders.targetCount) {
            return false;
        }
    ...
    // НОВАЯ ФУНКЦИЯ: Получение прогресса рабочего времени (0-1)
    getWorkTimeProgress() {
